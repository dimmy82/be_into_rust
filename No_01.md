...
```rust
    ...
    let s1: String = String::from("hello");
    takes_ownership(s1);                    // 所有権はs1からfunction:takes_ownershipのtargetに移譲した
    println!("{}, world!", s1);             // s1は無効なのでコンパイルエラーになる
```
...
```rust
    fn takes_ownership_and_give_back(target: String) -> (String, usize) {
        let length = target.len();
        (target, length)                                   // targetで受け取った所有権をそのまま返す
    }
